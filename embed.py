...
from retrieval_quality import build_embedding_text, enrich_chunk_metadata, score_candidate


CHUNKS_FILE = os.getenv("BIOMIND_CHUNKS_FILE", "chunks.jsonl")
DB_DIR = os.getenv("BIOMIND_DB_DIR", "biomind_db")
...
CHROMA_BATCH_SIZE = int(os.getenv("BIOMIND_CHROMA_BATCH_SIZE", "200"))

TOP_K = int(os.getenv("BIOMIND_TOP_K", "4"))
WINDOW = int(os.getenv("BIOMIND_WINDOW", "0"))
CANDIDATE_MULTIPLIER = int(os.getenv("BIOMIND_CANDIDATE_MULTIPLIER", "8"))
...
def load_chunks() -> list[dict[str, Any]]:
    ...
    with caminho.open(encoding="utf-8") as arquivo:
        for numero_linha, linha in enumerate(arquivo, start=1):
            ...
            # Enriquece também arquivos chunks.jsonl antigos. Assim, reconstruir
            # o índice já ativa classificação e reranking sem exigir que o
            # chunking seja refeito primeiro.
            quality_metadata = enrich_chunk_metadata(
                str(chunk["source"]),
                str(chunk["text"]),
                str(chunk.get("section") or "") or None,
            )
            for key, value in quality_metadata.items():
                chunk.setdefault(key, value)
            chunk.setdefault(
                "embedding_text",
                build_embedding_text(str(chunk["source"]), str(chunk["text"]), chunk),
            )
            chunks.append(chunk)
...
def metadata_for(chunk: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "document_type": str(chunk.get("document_type", "educational_material")),
        "content_type": str(chunk.get("content_type", "general")),
        "topic": str(chunk.get("topic", "general_trichology")),
        "source_quality": str(chunk.get("source_quality", "medium")),
...
def retrieve(pergunta: str) -> list[dict[str, Any]]:
    ...
    for record_id, metadata, distancia in zip(
        ...
        texto_mesclado = merge_overlapping_chunks(textos)
        quality = score_candidate(
            pergunta,
            semantic_similarity=similaridade,
    ...
    resultados.sort(
        key=lambda item: (float(item.get("adjusted_score", 0.0)), float(item["similarity"])),
        reverse=True,
    )
...
def ask(pergunta: str) -> None:
    ...
    for resultado in resultados:
        ...
        print(
            ...
            f"· sim {resultado['similarity']:.3f} · score {resultado.get('adjusted_score', 0):.3f}"
        )
