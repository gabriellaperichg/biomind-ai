...
from retrieval_quality import build_embedding_text, enrich_chunk_metadata


PDF_DIR = "pdfs"
OUT_FILE = "chunks.jsonl"

MAX_CHARS = int(os.getenv("BIOMIND_CHUNK_MAX_CHARS", "1200"))
OVERLAP_CHARS = int(os.getenv("BIOMIND_CHUNK_OVERLAP_CHARS", "160"))
MAX_UNIT_CHARS = int(os.getenv("BIOMIND_MAX_UNIT_CHARS", "1800"))
...
BOILERPLATE_PATTERNS = [
    ...
    # Remove identificação/licença nominal que polui os embeddings e não
    # acrescenta valor clínico ao RAG.
    r"Conte[uú]do licenciado para.{0,250}?(?:\d{3}\.\d{3}\.\d{3}-\d{2}|$)",
    r"Licenciado para.{0,250}?(?:\d{3}\.\d{3}\.\d{3}-\d{2}|$)",
]
...
SECTION_START_RE = re.compile(
    r"^(?:[-•]\s*)?(?:conduta|tratamento|terap[eê]utica|diagn[oó]stico|"
    r"hip[oó]teses?|exames?|avalia[cç][aã]o|anamnese|procedimentos?|"
    r"efeitos? colaterais?|eventos? adversos?|complica[cç][oõ]es?|"
...
def limpar_texto(texto: str) -> str:
    ...
    # Preserva marcadores e títulos clínicos como fronteiras semânticas.
    # Isso evita que descrição do caso, diagnóstico e tratamento terminem no
    # mesmo chunk apenas porque cabem no limite de caracteres.
    texto = re.sub(
        r"(?im)^\s*[•●▪◦*-]\s+",
        "\n\n- ",
        texto,
    ...
    texto = re.sub(
        r"(?i)\s+(?=(?:conduta|tratamento|terap[eê]utica|diagn[oó]stico|"
        r"hip[oó]teses?|exames?|avalia[cç][aã]o|anamnese|procedimentos?|"
        r"efeitos? colaterais?|eventos? adversos?|complica[cç][oõ]es?|"
        r"observa[cç][oõ]es?)\s*:)",
        "\n\n",
...
def montar_chunk(
    ...
    quality_metadata = enrich_chunk_metadata(source, texto)
    ...
    chunk["embedding_text"] = build_embedding_text(source, texto, quality_metadata)
    return chunk
...
def empacotar_unidades(
    ...
    for unidade in unidades:
        # Títulos como "Exames:" e "Conduta:" iniciam uma nova unidade
        # semântica mesmo quando o documento inteiro caberia em um único
        # chunk. O overlap não atravessa essa fronteira para não misturar
        # descrição/diagnóstico com tratamento.
        starts_section = bool(SECTION_START_RE.match(unidade.text.strip()))
        if atual and starts_section:
            chunks.append(
